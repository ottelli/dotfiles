...
local icon_dir = "~/.config/awesome/icons/network/"


-- Factory function for producing networking widget
local function factory(args)
    ...
    function wirelessStatus:getInterface()
        awful.spawn.easy_async_with_shell(
            "awk 'NR==3 {print $1}' /proc/net/wireless",
...
-- Setup a widget
local wirelessStatus = factory({
    notification_preset = { font = "Mononoki Nerd Font 10", fg = "#ffffff" },
    timeout = 30,
    settings = function(self)
        if self.status == "1" or self.status == "" then
            self.widget:set_image(icon_dir.."wireless_na.png")
        else
            if self.perc <= 5 then
                self.widget:set_image(icon_dir.."wireless_0.png")
            elseif self.perc <= 25 then
                self.widget:set_image(icon_dir.."wireless_1.png")
            elseif self.perc <= 50 then
                self.widget:set_image(icon_dir.."wireless_2.png")
            elseif self.perc <= 75 then
                self.widget:set_image(icon_dir.."wireless_3.png")
            else
                self.widget:set_image(icon_dir.."wireless_3.png")
            end
        end
...
-- -- Return instantiated widget with a layout
-- return wibox.widget {
--     wirelessStatus.widget,
...
return wibox.widget {
    ...
    -- utils.separator.minor,
    -- awful.widget.watch('bash -c "awk \'NR==3 {printf("%.0f", $3*10/7)}\' /proc/net/wireless"', 60),
